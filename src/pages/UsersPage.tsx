...
import { getAllUsersFull, deleteUser, updateUser, getQrCode, restartSession, stopSession, getServers } from '@/lib/api';
...
export default function UsersPage() {
    ...
    switch (s) {
      ...
      case 'maxqrcodetries':
        return (
          <div className="flex items-center gap-1.5 text-warning">
            <WifiOff className="w-4 h-4" />
            <span className="text-[10px] font-bold uppercase tracking-wider">MaxQR</span>
          </div>
        );
      case 'none':
        return (
          <div className="flex items-center gap-1.5 text-muted-foreground">
            <WifiOff className="w-4 h-4" />
            <span className="text-[10px] font-bold uppercase tracking-wider">none</span>
          </div>
        );
      default:
        return (
          <div className="flex items-center gap-1.5 text-destructive">
            <WifiOff className="w-4 h-4" />
            <span className="text-[10px] font-bold uppercase tracking-wider">{status || 'Not Ready'}</span>
          </div>
        );
    }
  };
  ...
  const filteredDevices = devices.filter((device) => {
    ...
    const matchesStatus = statusFilter === 'all' || String(device.status).toLowerCase() === statusFilter.toLowerCase();
    const matchesServer = serverFilter === 'all' || device.server_code === serverFilter || device.server_name === serverFilter;

    // Customer Number Range
    let matchesCustomerRange = true;
    if (customerFrom) matchesCustomerRange = matchesCustomerRange && Number(device.customer_number) >= Number(customerFrom);
    if (customerTo) matchesCustomerRange = matchesCustomerRange && Number(device.customer_number) <= Number(customerTo);
    ...
    if (dateFilter !== 'all' && device.last_message_date) {
      ...
      const now = new Date();
      if (dateFilter === 'hour') matchesDate = isAfter(lastDate, subHours(now, 1));
      else if (dateFilter === 'day') matchesDate = isAfter(lastDate, subDays(now, 1));
      else if (dateFilter === 'week') matchesDate = isAfter(lastDate, subWeeks(now, 1));
    }

    return matchesSearch && matchesOrg && matchesStatus && matchesServer && matchesCustomerRange && matchesDate;
  });
  ...
  const confirmDelete = async () => {
    ...
    const userId = String(selectedDevice.session_id || selectedDevice.user_code);
    try {
      ...
      >
        <div className="overflow-x-auto">
          <table className="w-full whitespace-nowrap">
            <thead>
              <tr>
                <th className="min-w-[100px] cursor-pointer hover:bg-muted" onClick={() => {/* handleSort implementation */}}>
                ...
                <th className="sticky left-0 bg-muted/50 z-10" style={{ backgroundColor: 'oklch(96.8% 0.007 247.896)' }}>إجراءات</th>
              </tr>
            </thead>
            <tbody>
              {paginatedDevices.map((device, index) => (
                <motion.tr
                  key={device.session_id || device.user_code}
                ...
                >
                  ...
                  <td className="font-mono text-xs">{device.customer_number}</td>
                  <td>
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-lg bg-primary/10 flex items-center justify-center shrink-0">
                        <Building2 className="w-3.5 h-3.5 text-primary" />
                      </div>
                      <span className="font-medium">{device.SOMNA || device.customer_name || 'بدون اسم'}</span>
                    </div>
                  </td>
                  ...
                  <td className="text-[10px]">
                    <div className="flex flex-col gap-0.5">
                      <div className="flex items-center gap-1 text-foreground/80">
                        ...
                        <span className="font-medium">الكل: {device.total_messages || 0}</span>
                      </div>
                      <div className="flex items-center gap-1 text-[9px] text-muted-foreground pr-3.5">
                        <span>({device.total_text || device.total_text_messages || 0} نص - {device.total_attachments || device.total_attachment_messages || 0} مرفق)</span>
                      </div>
                    </div>
                  </td>
                  <td className="text-[10px]">
                    <div className="flex flex-col gap-0.5">
                      <div className="flex items-center gap-1 text-primary">
                        ...
                        <span className="font-medium">اليوم: {device.today_messages || 0}</span>
                      </div>
                      <div className="flex items-center gap-1 text-[9px] text-primary/70 pr-3.5">
                        <span>({device.today_text || device.today_text_messages || 0} نص - {device.today_attachments || device.today_attachment_messages || 0} مرفق)</span>
                      </div>
                    </div>
                  </td>
                  <td className="font-mono text-[10px] uppercase">{device.server_code || '-'}</td>
                  <td className="sticky left-0 bg-background/95 backdrop-blur-sm border-r z-10">
                    <div className="flex items-center gap-1">
                      ...
                      <Button
                        ...
                        disabled={restartingIds.has(String(device.session_id || device.user_code))}
                        title="إعادة تشغيل"
                      >
                        {restartingIds.has(String(device.session_id || device.user_code)) ? (
        ...
        <DialogContent className="sm:max-w-md text-right" dir="rtl">
          <DialogHeader>
            ...
            <DialogDescription className="text-right pt-2 text-foreground font-medium">
              هل أنت متأكد من رغبتك في حذف الجلسة <span className="font-mono bg-muted px-1.5 py-0.5 rounded text-destructive">{selectedDevice?.session_id}</span>؟
            </DialogDescription>
          </DialogHeader>
